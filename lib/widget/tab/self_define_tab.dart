...
///
///  * [TabBar], which displays a row of tabs.
///  * [ZTabBarView], which displays a widget for the currently selected tab.
///  * [TabController], which coordinates tab selection between a [ZTabBar] and a [ZTabBarView].
///  * <https://material.io/design/components/tabs.html>
class ZTab extends StatelessWidget {
  /// Creates a material design [ZTabBar] tab.
  ///
...
class WrapTab extends StatefulWidget {
  const WrapTab({Key key, this.labelPadding, this.tab}) : super(key: key);
  final EdgeInsetsGeometry labelPadding;
  final Widget tab;

  @override
  _WrapTabState createState() => _WrapTabState();
}

class _WrapTabState extends State<WrapTab> {
  @override
  Widget build(BuildContext context) {
...
//    if (kSelfTabHeight > 0) {
//      return Size.fromHeight(kSelfTabHeight + indicatorWeight);
//    } else {
...
class _TabBarState extends State<ZTabBar> {
...
//  List<GlobalKey<_WrapTabState>> tabTextKeys2 = new List<GlobalKey>();
  List<GlobalKey> tabTextKeys;
  ...
  void initState() {
    ...
    tabKeys = widget.tabs.map((Widget tab) => GlobalKey()).toList();
    HuoLog.d("tabKeys: ${tabKeys.toString()}");
    tabTextKeys = widget.tabs.map((Widget tab) => GlobalKey()).toList();
    HuoLog.d("tabTextKeys: ${tabTextKeys}");
  }
  ...
  @override
  void didUpdateWidget(ZTabBar oldWidget) {
    ...
    if (widget.tabs.length > oldWidget.tabs.length) {
      ...
      tabKeys.addAll(List<GlobalKey>.generate(delta, (int n) => GlobalKey()));
//      tabTextKeys
//          .addAll(List<GlobalKey>.generate(delta, (int n) => GlobalKey()));
    } else if (widget.tabs.length < oldWidget.tabs.length) {
      tabKeys.removeRange(widget.tabs.length, oldWidget.tabs.length);
//      tabTextKeys.removeRange(widget.tabs.length, oldWidget.tabs.length);
    }
  }
...
//      GlobalKey<_WrapTabState> key = new GlobalKey();
//      tabTextKeys2.add(key);
//      wrappedTabs[i] = WrapTab(
//        key: key,
//        labelPadding:
//            widget.labelPadding ?? tabBarTheme.labelPadding ?? kTabLabelPadding,
//        tab: widget.tabs[i],
//      );
//      tabTotalSize += wrappedTabs[i].createElement().size.width;
//      HuoLog.d("tabTotalSize: ${tabTotalSize.toString()}");
    }
    ...
    double tabWidth = 0;
//    HuoLog.d("tabTextKeys2: ${tabTextKeys2}");
//    for (int index = 0; index < tabCount; index += 1) {
//      final RenderBox box =
//          tabTextKeys2[index].currentContext.findRenderObject();
//      HuoLog.d("box*******************: ${box.toString()}");
//      tabWidth = box.size.width;
//      tabWidth += tabWidth;
//    }
//    //rightPadding=(maxWidth-tabs.length.width)/tabs.length
//    rightPadding = (screenWidth - tabWidth) / tabCount;
    for (int index = 0; index < tabCount; index += 1) {
...
//      final RenderBox box =
//      tabKeys[index].currentContext.findRenderObject();
//      tabWidth = box.size.width;
//      tabWidth += tabWidth;
//      HuoLog.d("tabWidth#################${tabWidth.toString()}");

//      if (!widget.isScrollable) {
//        if (!widget.isAdjustWeight) {
//          //根据内容来适配,获取内容的宽度控制右边距rightPadding=(maxWidth-tabs.length.width)/tabs.length
...
class _TabBarViewState extends State<ZTabBarView> {
  TabController _controller;
  ...
  @override
  void didUpdateWidget(ZTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
