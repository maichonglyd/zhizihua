...
void _init(Action action, Context<MineFragmentState> ctx) {
  ...
  scrollController.addListener(() {
    ...
    if (scrollController.offset > 130 && ctx.state.isExpand) {
      ctx.dispatch(MineFragmentActionCreator.updateExpand(false));
    }
    if (scrollController.offset < 130 && !ctx.state.isExpand) {
      ctx.dispatch(MineFragmentActionCreator.updateExpand(true));
    }
  });

  ctx.dispatch(MineFragmentActionCreator.onCreateController(scrollController));
}
